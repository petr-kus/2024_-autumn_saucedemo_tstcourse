from POM.common import click_on_element, get_elements_by_xpath, get_element_by_id
import random
from .base import LoggedInPage

class InventoryPage(LoggedInPage):
    page_url = 'inventory.html'
    id_cart_icon = "shopping-cart-link"
    add_to_cart_xpath = "xpath=//button[contains(.,'Add to cart')]"
    ...
    def add_to_cart_random(self):
        ...
        elements = get_elements_by_xpath(self.driver, self.add_to_cart_xpath)
        for index in random_indexes:
            click_on_element(self.driver, elements, index)
        self.take_screenshot(self.driver,self.screenshot_folder)
        return random_integer # expected number of items in the cart
    
    def click_cart_icon(self):
        get_element_by_id(self.driver, self.id_cart_icon, 'cart icon').click()
